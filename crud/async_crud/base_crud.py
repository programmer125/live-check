...
from db.async_session import default_db, neoailive_db
from db.session import SessionLocalAsync, NeoailiveSessionLocalAsync
...
class CRUDBase(Generic[ModelType]):
    def __init__(self, model: type[ModelType], engine="default"):
        self.model = model
        if engine == "neoailive":
            self.db = neoailive_db
        else:
            self.db = default_db
        self.engine = engine
    ...
    def as_dict(self, obj, fields=None):
        ...
        get_key_value = lambda c: (
            (c, getattr(obj, c).strftime("%Y-%m-%d %H:%M:%S"))
    ...
    def get_session(self):
        if self.engine == "neoailive":
            return NeoailiveSessionLocalAsync()
        else:
    ...
    async def create(
        ...
        *,
        ...
        db_session = db_session or self.get_session() or self.db.session
    ...
    async def fetch_all(
        ...
        **filters,
        ...
        async with db_session.begin():
            response = await db_session.execute(
                query.where(*wheres).filter_by(**filters)
            )
